...
<div class=meta>本金階梯：餘額 ${s.equity&&s.equity.balance!=null?s.equity.balance.toFixed(1)+' U':'（未讀取' + (s.equity&&s.equity.error?'：'+s.equity.error:'') + '）'} → 下單用 <b>${s.equity?s.equity.tier:'—'} U</b>（基準 ${s.equity?s.equity.base:''}，每級 ${s.equity?s.equity.step:''}，上限 ${s.equity?s.equity.cap:''}）</div>
...
<h2>訊號（最新在上） <button onclick="sigclear()" style="font-size:12px;padding:3px 8px">清除訊號紀錄</button></h2>${T(sig,['time','symbol','engine','side','entry','stop','stop_pct','equity','leverage','notional','executed','reason'],r=>r.side=='LONG'?'long':'short')}
