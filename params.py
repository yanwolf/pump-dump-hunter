...
SCHEMA = [
  ...
  dict(g="F 崩盤進行中追空（1m，清算連鎖）", k="ENGINE_F.red_bars", label="連黑根數", unit="根1m", step=1,
       help="最近幾根全部收黑（3）"),
  dict(g="F 崩盤進行中追空（1m，清算連鎖）", k="ENGINE_F.stop_bars", label="止損回看", unit="根1m", step=1,
       help="止損放最近幾根 1m 的高點（5）。調大：止損寬、R 小但不易被掃"),
  dict(g="F 崩盤進行中追空（1m，清算連鎖）", k="EXIT.F.max_hold_bars", label="最長持有", unit="分鐘", step=15,
