...
if os.path.exists(_path):
    try:
        with open(_path, encoding="utf-8") as f: _state.update(json.load(f))
    except Exception as e:
        bad = f"{_path}.bad-{int(time.time())}"
        try: os.replace(_path, bad); bad = os.path.basename(bad)
        except Exception: bad = "（另存失敗）"
        LOAD_ERROR = (f"狀態檔讀取失敗 {type(e).__name__}: {e}；壞檔另存為 {bad}。持倉、成交界線、待平倉、pending 可能遺失，"
                      "請對照交易所確認")
        _state["errors"].append(f"{time.strftime('%m-%d %H:%M')} {LOAD_ERROR}")
_fail = dict(n=0)
